/**
 * @file octave_filter_bank.cpp
 * @brief Implementation of the OctaveFilterBank class (multi-band IIR analysis).
 *
 * See octave_filter_bank.hpp for API documentation.
 */
 ...
 // -----------------------------------------------------------------------------
 // Static member definition: octave-band center frequencies (Hz).
 // Order corresponds to band indices 0..6.
 // -----------------------------------------------------------------------------
 ...
 void OctaveFilterBank::reset() {
     // Zero all delay-line states in every biquad section.
 ...
 double OctaveFilterBank::processBiquad(BiquadSection& section, double input) {
     // Direct Form I (state in z[0], z[1]):
     // y[n] = b0*x[n] + z1
     // z1  = b1*x[n] - a1*y[n] + z2
     // z2  = b2*x[n] - a2*y[n]
 ...
 double OctaveFilterBank::processFilterBand(FilterBand& band, double input) {
     // Cascade the configured number of biquad sections.
     double output = input;
 ...
 std::vector<std::vector<double>> OctaveFilterBank::process(const std::vector<float>& input) {
     if (!initialized_) {
         throw std::runtime_error("Filter bank has not been initialized.");
     }
     ...
     // Prepare output container: one vector per band, same length as input.
     std::vector<std::vector<double>> output(NUM_BANDS, std::vector<double>(input.size()));
 
     // Sample-by-sample processing through every band.
 ...
 void OctaveFilterBank::initializeCoefficients() {
     // Initialize center frequencies (section vectors resized below).
